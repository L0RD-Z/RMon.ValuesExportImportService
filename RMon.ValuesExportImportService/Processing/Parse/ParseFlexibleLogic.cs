    ...
    class ParseFlexibleLogic
    {
        ...
        public async Task<List<ValueInfo>> AnalyzeAsync(IList<LocalFile> files, ParseProcessingContext context, CancellationToken ct)
        {
            ...
            foreach (var table in tables)
            {
                ...
                foreach (var row in table.Item2.Entities)
                {
                    rowNumber++;
                    try
                    {
                    ...
                    {
                        await context.LogWarning(e.ConcatExceptionMessage(TextParse.RowParseError.With(table.FileName, rowNumber))).ConfigureAwait(false);
                    }
                }
            }
